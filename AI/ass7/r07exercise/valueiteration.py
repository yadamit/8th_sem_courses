...
def value_of(mdp, s, a, v, gamma):
    ...
    for s2 in mdp.successor_states(s,a):
        sum += mdp.P(s,a,s2) * ( mdp.R(s,a,s2) + gamma*v[s2] )
    return sum
...
def value_iteration(mdp, gamma, epsilon = 0.001):
    ...
    for _ in range(100): #TODO: change this to while(True)  
        v_new = v.copy()
        act_values = {}
        for s in mdp.states():
            for a in mdp.applicable_actions(s):
                act_values[a] = value_of(mdp,s,a,v,gamma)

            v_new[s] = act_values[ argmax(act_values) ]
        # break condition
        # print([ ((v_new[s]-v[s])<=epsilon) and (v_new[s]-v[s])>=-epsilon for s in mdp.states()])
        if [ ((v_new[s]-v[s])<epsilon) and (v_new[s]-v[s])>-epsilon for s in mdp.states()] == [True for s in mdp.states()]:
            break
        v = v_new
