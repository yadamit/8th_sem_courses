...
def minimax(player,state,depthLeft):
  ...
  global calls
  calls += 1
  if depthLeft == 0 or not state.applicableActions(player):
    return state.value()
  player2 = 1-player
  best = float('inf') if player==0 else -float('inf')
  for a in state.applicableActions(player):
     s_ = state.successor(player, a)
     val = minimax(player2, s_, depthLeft-1)
     best = min(val, best) if player==0 else max(val, best)
  return best
...
def alphabeta(player,state,depthLeft,alpha,beta):
  ...
  for a in state.applicableActions(player):
     s_ = state.successor(player, a)
     val = alphabeta(player2, s_, depthLeft-1, alpha, beta)
     if player==0:
        best = min(val, best)
        beta = min(beta, val)
     else:
