...
class KnightState:
    ...
    def successors(self):
        # move some night
        succ = []
        actions = ["up right", "up left", "down right", "down left", "right up", "right down", "left up", "left down"]
        x = [2, 2,-2,-2,1,-1, 1,-1]
        y = [1,-1, 1,-1,2, 2,-2, -2]
        for knight in range(len(self.occupied)):
            for act_i in range(len(actions)):
                new_locations = self.occupied.copy()
                # print("new_locations: ",new_locations)
                new_locations[knight] = (new_locations[knight][0]+x[act_i], new_locations[knight][1]+y[act_i], new_locations[knight][2])
                if new_locations[knight][0] < 8 and new_locations[knight][0] >= 0:
                    if new_locations[knight][1] < 8 and new_locations[knight][1] >= 0:
                        already_a_knight = False
                        for (a,b,c) in self.occupied:
                            if (a,b) == (new_locations[knight][0:-1]):
                                already_a_knight = True
                        if not already_a_knight:
                            action = "moving " + str(self.occupied[knight]) + " to " + str(new_locations[knight])
                            # new_locations = self.occupied.copy()
                            # new_locations[knight] = (new_locations[knight][0]+x[act_i], new_locations[knight][1] + y[act_i],new_locations[knight][2])
                            # new_locations[knight][1] += y[act_i]
                            # action += str(new_locations[knight])
                            new_state = KnightState(new_locations)
...
# breadthFirstSearch(KnightState([(0,0,True),(1,0,True)]),
#                    lambda state: (state.occupied == [(0,0,True),(1,0,True)]))
# exit()
